...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * - 이때, 호출하는 메서드를 Caller, 호출 당하는 메서드를 Worker 라고함
		 * - main() 메서드도 메서드의 일종
		 ...
		 *   이때, 리턴값이 존재한다면 호출한 곳으로 돌아올때 가지고 옴
		 * 
		...
		// 1. 매개변수 (X), 리턴 (X)
		// => 매개변수가 없으므로 메서드 호출시 소괄호() 안에 아무 데이터도 전달하지 않음
		// => 리턴값이 없으므로 메서드 호출결과를 저장할 변수를 선언하거나
		//    출력문 등의 내부에서 메서드 호출 불가능하며, 단돈으로 호출해야한다!
...
//		System.out.println("동생이 사다준것 : " + "새우깡"); 과 동일한 코드로 사용됨.
	...
	public static String sister_2() {
		System.out.println("동생: 오빠가 물을 떠오라고 시켯다!");
